...
Access real-time buoy readings and information from the National Data Buoy Center (NDBC) with this API.

Learn more about NDBC Buoy Data API at [surftruths.com/api.html](https://surftruths.com/api.html).
...
This SDK exposes the API as a small set of **semantic entities** — Buoy — that you
...
const items = await client.Buoy().list()
```
...
```go
client := sdk.Test()
result, err := client.Buoy(nil).List(
    nil, nil,
)
```
