...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("NDBC_BUOY_DATA_TEST_LIVE") == "TRUE" ||
		os.Getenv("NDBC_BUOY_DATA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("NDBC_BUOY_DATA_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.NdbcBuoyDataSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.NdbcBuoyDataError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.NdbcBuoyDataSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.NdbcBuoyDataError{Msg: msg, Code: code}
}
