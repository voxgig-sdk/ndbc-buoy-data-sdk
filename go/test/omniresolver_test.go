...
import (
	...
	omni "github.com/voxgig-sdk/ndbc-buoy-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NdbcBuoyDataError); is {
				out := map[string]any{
					"name":    "NdbcBuoyDataError",
...
func omniCtx(arg any, client *sdk.NdbcBuoyDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NdbcBuoyDataSDK); is {
			client = live
