...
func TestBuoyEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Buoy(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Buoy(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set NDBC_BUOY_DATA_TEST_BUOY_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		buoyRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.buoy")))
...
func buoyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NDBC_BUOY_DATA_TEST_BUOY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NDBC_BUOY_DATA_TEST_BUOY_ENTID"])
