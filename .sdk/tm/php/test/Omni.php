...
//    only - a typed NdbcBuoyDataContext there would make every ctx
...
final class NdbcBuoyDataOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new NdbcBuoyDataContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new NdbcBuoyDataSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new NdbcBuoyDataResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new NdbcBuoyDataError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new NdbcBuoyDataResponse($clean['response']);
    ...
    public static function err_from_map($m): ?NdbcBuoyDataError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NdbcBuoyDataError(is_string($code) ? $code : '', $msg);
    }
