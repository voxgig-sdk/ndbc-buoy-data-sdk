...
// NdbcBuoyData SDK retry feature
...
class NdbcBuoyDataRetryFeature extends NdbcBuoyDataBaseFeature
{
    ...
    public function init(NdbcBuoyDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NdbcBuoyDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NdbcBuoyDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NdbcBuoyDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
