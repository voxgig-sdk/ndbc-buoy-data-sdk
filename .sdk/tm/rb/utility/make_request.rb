...
module NdbcBuoyDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NdbcBuoyDataResponse.new({})
    ...
    if fetch_err
      ...
      response = NdbcBuoyDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NdbcBuoyDataResponse.new(fetched)
    else
