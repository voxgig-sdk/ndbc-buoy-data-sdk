...
from projectname_sdk import NdbcBuoyDataSDK
from projectname_sdk.core.spec import NdbcBuoyDataSpec
from projectname_sdk.core.result import NdbcBuoyDataResult
from projectname_sdk.core.response import NdbcBuoyDataResponse
from projectname_sdk.core.operation import NdbcBuoyDataOperation
from projectname_sdk.core.error import NdbcBuoyDataError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import NdbcBuoyDataBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return NdbcBuoyDataError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = NdbcBuoyDataSDK.test(None, None)
        ...
        class TestInitFeature(NdbcBuoyDataBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(NdbcBuoyDataBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = NdbcBuoyDataSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = NdbcBuoyDataSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = NdbcBuoyDataSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = NdbcBuoyDataOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = NdbcBuoyDataSDK.test(None, setup_opts)
