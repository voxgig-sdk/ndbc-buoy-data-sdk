...
describe("BuoyDirect", function()
  it("should direct-list-buoy", function()
    local setup = buoy_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-buoy", setup.live and "live" or "unit")
  ...
  it("should direct-load-buoy", function()
    local setup = buoy_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-buoy", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
