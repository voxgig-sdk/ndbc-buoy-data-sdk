# NdbcBuoyData Lua SDK Reference

Complete API reference for the NdbcBuoyData Lua SDK.
...
```lua
local results, err = client:Buoy(nil):list(nil, nil)
```
...
```lua
local result, err = client:Buoy(nil):load({ id = "buoy_id" }, nil)
```
