...
local BuoyEntity = {}
BuoyEntity.__index = BuoyEntity


function BuoyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BuoyEntity)
  self._name = "buoy"
  self._client = client
...
function BuoyEntity:get_name()
  return self._name
end


function BuoyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BuoyEntity.new(self._client, opts)
end
...
function BuoyEntity:mark_deleted()
  self._deleted = true
end


function BuoyEntity:deleted()
  return true == self._deleted
end


function BuoyEntity:data_set(args)
  if args ~= nil then
...
function BuoyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BuoyEntity:match_set(args)
  if args ~= nil then
...
function BuoyEntity:stream(action, args, callopts)
...
function BuoyEntity:_run_op(ctx, post_done)
  local utility = self._utility
