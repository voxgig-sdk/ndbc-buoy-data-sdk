...
[MCP](https://modelcontextprotocol.io) server exposing the NdbcBuoyData SDK as
two agent tools — `ndbc-buoy-data_list` and `ndbc-buoy-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ndbc-buoy-data-mcp)
make build

# 2. Provide credentials via the environment
export NDBC_BUOY_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ndbc-buoy-data \
  -- /absolute/path/to/ndbc-buoy-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ndbc-buoy-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ndbc-buoy-data_list: first page of records
{ "entity": "buoy" }
{ "entity": "buoy", "query": { } }

// ndbc-buoy-data_load: one record by id
{ "entity": "buoy", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ndbc-buoy-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ndbc-buoy-data \
     -- "$PWD"/dist/*/ndbc-buoy-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ndbc-buoy-data_list` and `ndbc-buoy-data_load` tools now appear
   in new sessions. Ask the agent to *"list buoy using ndbc-buoy-data"*
   and it calls `ndbc-buoy-data_list` with `{"entity":"buoy"}`.
...
```sh
export NDBC_BUOY_DATA_APIKEY=sk_live_xxx            # API key
export NDBC_BUOY_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ndbc-buoy-data_list` tool
...
### Call the `ndbc-buoy-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ndbc-buoy-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ndbc-buoy-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NDBC_BUOY_DATA_APIKEY` | API key sent with every request. |
| `NDBC_BUOY_DATA_BASE` | Optional override of the API base URL. |
...
```sh
./ndbc-buoy-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ndbc-buoy-data_load","arguments":{"entity":"buoy","query":{"id":1}}}}'
```
