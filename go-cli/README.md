...
boru-driven command-line client **and** interactive REPL for the NdbcBuoyData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ndbc-buoy-data-cli)
make build

# 2. See usage (words, entities, env vars)
./ndbc-buoy-data-cli --help

# 3. Provide credentials once, via the environment
export NDBC_BUOY_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ndbc-buoy-data-cli list buoy
./ndbc-buoy-data-cli load 1 buoy            # {id:1} shorthand
./ndbc-buoy-data-cli load '{id:1}' buoy       # explicit match map

# 5. Override the API base URL for a single call
NDBC_BUOY_DATA_BASE=https://api.example.com ./ndbc-buoy-data-cli list buoy

# 6. No arguments -> interactive REPL
./ndbc-buoy-data-cli
ndbc-buoy-data> list buoy
ndbc-buoy-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ndbc-buoy-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ndbc-buoy-data-cli list buoy
   ```
...
```sh
./ndbc-buoy-data-cli load 1 buoy          # scalar shorthand for {id:1}
./ndbc-buoy-data-cli load '{id:1}' buoy     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `ndbc-buoy-data>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `buoy`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ndbc-buoy-data-cli`. |
...
(`./ndbc-buoy-data-cli <expr>`) and interactively (the REPL), and expressions compose the
