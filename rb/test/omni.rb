...
module NdbcBuoyDataOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if NdbcBuoyDataOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = NdbcBuoyDataOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| NdbcBuoyDataOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || NdbcBuoyDataOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = NdbcBuoyDataSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = NdbcBuoyDataError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { NdbcBuoyDataOmni.errify(err) }
