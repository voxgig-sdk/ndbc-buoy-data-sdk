...
class BuoyEntity
  ...
  def make
    opts = @_entopts.dup
    BuoyEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NdbcBuoyDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Buoy, Hash] the current Buoy data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NdbcBuoyDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Buoy fields)
  def match_get
  ...
  # Load a single Buoy.
  #
  # @param reqmatch [BuoyLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Buoy.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Buoy, Hash] the loaded Buoy; raises NdbcBuoyDataError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NdbcBuoyDataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [BuoyListMatch, Hash, nil] match filter (any subset of
  #   Buoy fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Buoy>, Array] the matching Buoy items; raises NdbcBuoyDataError on failure
