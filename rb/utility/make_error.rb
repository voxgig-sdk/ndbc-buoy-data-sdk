...
module NdbcBuoyDataUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = NdbcBuoyDataContext.new({}, nil)
    end
    op = ctx.op || NdbcBuoyDataOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || NdbcBuoyDataResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(NdbcBuoyDataError) ? err.msg : err.to_s
    ...
    sdk_err = NdbcBuoyDataError.new("", msg, ctx)
    sdk_err.result = ctx.utility.clean.call(ctx, result)
    sdk_err.spec = ctx.utility.clean.call(ctx, spec)
    sdk_err.code = err.code if err.is_a?(NdbcBuoyDataError)
