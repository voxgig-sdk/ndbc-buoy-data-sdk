...
class NdbcBuoyDataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NdbcBuoyDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NdbcBuoyDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NdbcBuoyDataTestRunner.getenv("NDBC_BUOY_DATA_TEST_LIVE")
        override = NdbcBuoyDataTestRunner.getenv("NDBC_BUOY_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NdbcBuoyDataTestRunner.getenv(key)
        ...
        explain = NdbcBuoyDataTestRunner.getenv("NDBC_BUOY_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if NdbcBuoyDataTestRunner._test_control is not None:
            return NdbcBuoyDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NdbcBuoyDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NdbcBuoyDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NdbcBuoyDataTestRunner.load_test_control()
...
def load_env_local():
    NdbcBuoyDataTestRunner.load_env_local()


def env_override(m):
    return NdbcBuoyDataTestRunner.env_override(m)


def entity_data(v):
    return NdbcBuoyDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NdbcBuoyDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NdbcBuoyDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NdbcBuoyDataTestRunner.load_test_control()


def live_client_options():
    return NdbcBuoyDataTestRunner.live_client_options()


def live_delay_ms():
    return NdbcBuoyDataTestRunner.live_delay_ms()
