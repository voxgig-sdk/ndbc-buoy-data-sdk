...
class TestBuoyEntity:
    ...
    def test_should_stream(self):
        ...
        base = NdbcBuoyDataSDK.test(seed, None)
        seen = list(base.Buoy(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = NdbcBuoyDataSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Buoy(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NDBC_BUOY_DATA_TEST_BUOY_ENTID JSON to run live")
        client = setup["client"]
...
def _buoy_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("NDBC_BUOY_DATA_TEST_BUOY_ENTID"))
    ...
    _live = env.get("NDBC_BUOY_DATA_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("NDBC_BUOY_DATA_TEST_EXPLAIN") == "TRUE",
        "live": _live,
