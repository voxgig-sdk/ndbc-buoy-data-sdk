...
from ndbcbuoydata_sdk.ndbcbuoydata_types import (
    Buoy,
    BuoyLoadMatch,
    BuoyListMatch,
)
...
class BuoyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BuoyEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Buoy:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Buoy().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Buoy]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Buoy().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
