...
//     (\`new NdbcBuoyDataSDK(...)\` / \`NdbcBuoyDataSDK.test(...)\` become
//     \`NdbcBuoyDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'NdbcBuoyDataSDK'
...
const TEST_SEED = {"entity":{"buoy":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NdbcBuoyDataSDK(...)\` and any
// \`NdbcBuoyDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NdbcBuoyDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NdbcBuoyDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
