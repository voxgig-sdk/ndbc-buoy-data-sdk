...
describe('BuoyDirect', async () => {
  ...
  test('direct-load-buoy', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-buoy', setup.live)) return
  ...
  test('direct-list-buoy', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-buoy', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NDBC_BUOY_DATA_TEST_BUOY_ENTID']
