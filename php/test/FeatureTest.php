...
class FtCtrl extends NdbcBuoyDataControl
{
...
class FtClient
{
    ...
    private NdbcBuoyDataUtility $utility;

    public function __construct(NdbcBuoyDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): NdbcBuoyDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NdbcBuoyDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NdbcBuoyDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NdbcBuoyDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NdbcBuoyDataConfig::make_feature($name);
    ...
    private static function buildUrl(NdbcBuoyDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NdbcBuoyDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NdbcBuoyDataResponse($fetched);
        }
    ...
    private function fail(NdbcBuoyDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NdbcBuoyDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
