...
// NdbcBuoyData SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class NdbcBuoyDataTestFeature extends NdbcBuoyDataBaseFeature
{
    ...
    public function init(NdbcBuoyDataContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (NdbcBuoyDataContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = NdbcBuoyDataParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (NdbcBuoyDataContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(NdbcBuoyDataContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = NdbcBuoyDataParam::call($ctx, $k);
