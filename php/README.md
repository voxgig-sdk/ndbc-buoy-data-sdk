# NdbcBuoyData PHP SDK



The PHP SDK for the NdbcBuoyData API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Buoy()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/ndbc-buoy-data-sdk/releases](https://github.com/voxgig-sdk/ndbc-buoy-data-sdk/releases)
...
```php
try {
    // load() returns the ENTITY — call data_get() for the Buoy record (throws on error).
...
```php
$client = NdbcBuoyDataSDK::test($testopts, $sdkopts);
```
...
```
php/
├── ndbcbuoydata_sdk.php          -- Main SDK class
...
The main class (`ndbcbuoydata_sdk.php`) exports the SDK class
...
// $buoy->data_get() now returns the buoy data from the last list
